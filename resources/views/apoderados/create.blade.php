...
@section('page-title', 'Crear Apoderado')

@section('page-header')
<div>
    <h1 class="text-2xl font-bold text-gray-900">Crear Apoderado</h1>
    <p class="text-gray-600 mt-1">Registra un nuevo apoderado</p>
</div>
@endsection

@section('content')
<form action="{{ route('apoderados.store') }}" method="POST" class="space-y-6">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="relacion_legal" id="relacion_legal" required
                    ...
                    <option value="Padre" {{ old('relacion_legal') == 'Padre' ? 'selected' : '' }}>Padre</option>
                    <option value="Madre" {{ old('relacion_legal') == 'Madre' ? 'selected' : '' }}>Madre</option>
                    <option value="Tutor" {{ old('relacion_legal') == 'Tutor' ? 'selected' : '' }}>Tutor</option>
                    <option value="Otro" {{ old('relacion_legal') == 'Otro' ? 'selected' : '' }}>Otro</option>
                </select>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto" id="alumnos-list">
            ...
            <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer alumno-row" data-ci="{{ $alumno->ci ?? '' }}" data-nombres="{{ $alumno->nombres ?? $alumno->nombre_completo }}" data-apellidos="{{ ($alumno->apellido_paterno ?? '') . ' ' . ($alumno->apellido_materno ?? '') }}" data-direccion="{{ $alumno->direccion ?? '' }}">
                <input type="checkbox" name="alumnos[]" value="{{ $alumno->id_alumno }}" class="rounded border-gray-300 text-gray-600 focus:ring-gray-500">
                <div class="ml-3">
            ...
            @empty
            <p class="text-sm text-gray-500">No hay alumnos registrados. <a href="{{ route('alumnos.create') }}" class="text-gray-900 underline">Crear alumno</a></p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex justify-end space-x-4">
        ...
        <button type="submit" class="px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors">
            <i class="fas fa-save mr-2"></i>Guardar Apoderado
        </button>
    </div>
</form>
@endsection
