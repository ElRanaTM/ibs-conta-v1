...
@section('title', 'Editar Categoría de Egreso')

@section('page-title', 'Editar Categoría')

@section('page-header')
<div>
    <h1 class="text-2xl font-bold text-gray-900">Editar Categoría: {{ $categoria->nombre }}</h1>
    <p class="text-gray-600 mt-1">Modifica la información de la categoría</p>
</div>
@endsection

@section('content')
<form action="{{ route('catalogos.categorias-egreso.update', $categoria->id_categoria) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent">{{ old('descripcion', $categoria->descripcion) }}</textarea>
            </div>
        </div>
    </div>
